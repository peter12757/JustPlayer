...
#define XNTOHLL XHTONLL
        ...
        struct UnMarshallable
        {
            ...
            virtual ~UnMarshallable() {}
            virtual std::ostream & trace(std::ostream & os) const
            { return os << "trace UnMarshallable [ not immplement ]"; }
        };

        struct IPackage:public UnMarshallable,Marshallable
        {
        };
...
// Marshallable helper
        ...
        inline const Unpack & operator >> (const Unpack & p, const UnMarshallable & m)
        {
            const_cast<UnMarshallable &>(m).unmarshal(p);
            return p;
        }

        struct Voidmable : public Package::Marshallable
        {
            virtual void marshal(Pack &) const {}
            virtual void unmarshal(const Unpack &) {}
        };

        struct Mulmable : public Package::IPackage
        {
            Mulmable(const Package::IPackage & m1, const Package::IPackage & m2)
                    : mm1(m1), mm2(m2) { }

            const Package::IPackage & mm1;
            const Package::IPackage & mm2;

            virtual void marshal(Pack &p) const          { p << mm1 << mm2; }
            virtual void unmarshal(const Package::Unpack &p) { (void)p;assert(false); }
            virtual std::ostream & trace(std::ostream & os) const { return os << mm1 << mm2; }
        };

        struct Mulumable : public Package::IPackage
        {
            Mulumable(Package::IPackage & m1, Package::IPackage & m2)
                    : mm1(m1), mm2(m2) { }

            Package::IPackage & mm1;
            Package::IPackage & mm2;

            virtual void marshal(Pack &p) const          { p << mm1 << mm2; }
            virtual void unmarshal(const Package::Unpack &p) { p >> mm1 >> mm2; }
            virtual std::ostream & trace(std::ostream & os) const { return os << mm1 << mm2; }
        };

        struct Rawmable : public Package::Marshallable
        {
            ...
            virtual void marshal(Package::Pack & p) const   { p.push(m_data, m_size); }
            virtual void unmarshal(const Package::Unpack &) { assert(false); }
...
//#endif
        ...
        template <class T1, class T2>
        inline const Package::Unpack& operator >> (const Package::Unpack& s, std::pair<const T1, T2>& p)
        {
            const T1& m = p.first;
        ...
        template <class T1, class T2>
        inline const Package::Unpack& operator >> (const Package::Unpack& s, std::pair<T1, T2>& p)
        {
            s >> p.first >> p.second;
            return s;
        }
...
// add by luowl, 20100118
        ...
        template <class T>
        inline const Package::Unpack& operator >> (const Package::Unpack& p, std::vector<T>& vec)
        {
...
// end, luowl
        ...
        template <class T>
        inline const Package::Unpack& operator >> (const Package::Unpack& p, std::set<T>& set)
        {
...
//and it could unmarshal list,vector etc..
        ...
        inline void PacketToString(const Package::Marshallable &objIn, std::string &strOut)
        {
            PackBuffer buffer;
            Pack pack(buffer);
            objIn.marshal(pack);
...
#ifdef HAS_EXCEPTION
        inline bool StringToPacket(const std::string &strIn, Package::UnMarshallable &objOut)
        {
            try {
...
#else
        inline bool StringToPacket(const std::string &strIn, Package::Marshallable &objOut)
{
	Unpack unpack(strIn.data(), strIn.length());
	objOut.unmarshal(unpack);
    return true;
