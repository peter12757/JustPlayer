...
#ifndef ANDROIDTEST_LINKEDLIST_H
#define ANDROIDTEST_LINKEDLIST_H
...
template <typename T> class LinkedList;	// 最外层：链表类（模板）提前声明
...
public:
    ...
    friend class LinkedList<T>;			// 声明友类，以使LinkedList<T>类的成员函数访问结点类的私有成员data和next
...
template <typename T> class LinkedList			// 单向链表类模板设计
{
public:
    LinkedList(int n=0, const T *array=NULL);		// 构造函数（含默认的构造函数、转换构造函数等）
    LinkedList(const LinkedList &list);				// (深)拷贝构造函数
    ~LinkedList();								// 析构函数
    LinkedList & operator=(const LinkedList &list);	// (深)赋值运算符函数
    ...
    Node<T> *add(const T &t);				// 插入一个结点成为新的首结点
    Node<T> *addBefore(const T &t);			// 在当前结点之前插入一个结点
    Node<T> *addAfter(const T &t);			// 在当前结点之后插入一个结点
    ...
    Node<T> *getTop(), *get(int n), *getBottom();	// 当前结点绝对定位：首结点、第n个结点(n从0起)、尾结点
 ...
 ********************************************************************/
template <typename T>
LinkedList<T>::LinkedList(int n, const T *array) : head(NULL), cur_node(NULL), num(0)	// 构造函数（含默认的构造函数、转换构造函数等）
{
    Node<T> *p;
    for(int i=n-1; i>=0; i--)
    {
...
template <typename T>
LinkedList<T>::LinkedList(const LinkedList &list) : head(NULL), cur_node(NULL), num(0)	// (深)拷贝构造函数
{
...
template <typename T>
LinkedList<T> & LinkedList<T>::operator=(const LinkedList &list)// (深)赋值运算符函数
{
...
template <typename T>
void LinkedList<T>::PutList(ostream &out) const	// 输出链表所有结点的数据到引用out所绑定的ostream对象，该对象可以是cout，也可以是文件对象
{
...
template <typename T>
void LinkedList<T>::GetList(istream &in)			// 从输入设备in获取数据，形成链表。
{
//	FreeList();
    T x;
    //char str[80];
...
template <typename T>
ostream & operator<<(ostream &out, const LinkedList<T> &list)		// 本函数不是成员函数，仅为一个普通的C++函数
{
    list.PutList(out);
    return out;
}
...
template <typename T>
istream & operator>>(istream &in, LinkedList<T> &list)			// 本函数不是成员函数，仅为一个普通的C++函数
{
    list.GetList(in);
    return in;
}
...
template <typename T>
int LinkedList<T>::Save(const char *filename)		// 将链表所有结点的数据写入指定文件
{
...
template <typename T>
int LinkedList<T>::Load(const char *filename)		// 从指定文件中读取数据，构造链表
{
    int n = size();
...
template <typename T>
void LinkedList<T>::ShowList() const				// 输出链表所有结点的数据（格式一：在一行中输出）
{
    cout << "head";
    for(Node<T> *p=head; p!=NULL; p=p->next)
...
template <typename T>
Node<T> *LinkedList<T>::add(const T &t)		// 插入一个结点成为新的首结点
{
...
template <typename T>
Node<T> *LinkedList<T>::addBefore(const T &t)	// 在当前结点之前插入一个结点
{
...
template <typename T>
Node<T> *LinkedList<T>::addAfter(const T &t)	// 在当前结点之后插入一个结点
{
    if(cur_node==NULL || head==NULL)
        return cur_node=NULL;
    Node<T> *p = new Node<T>(t);
...
template <typename T>
Node<T> *LinkedList<T>::append(const T &t)		// 追加一个结点成为新的尾结点
{
...
template <typename T>
void LinkedList<T>::FreeList()					// 释放链表中的所有结点
{
    Node<T> *p;
    while(head!=NULL)
    {
...
template <typename T>
T LinkedList<T>::DeleteCurNode() throw(int)		// 删除当前结点，其下一个结点（成为新的当前结点），值返回被删结点数据域数值
{
    if(cur_node==NULL || head==NULL)
        throw -1;
    Node<T> *p;
...
template <typename T>
T LinkedList<T>::ModifyData(const T &t) throw(int)// 修改(替换)当前结点的数据域数值
{
    if(cur_node==NULL)
        throw -1;
...
template <typename T>
int LinkedList<T>::size() const				// 返回结点数
{
    return num;
}

template <typename T>
int LinkedList<T>::CurPos() const					// 返回当前结点的序号（-1表示无当前结点）
{
    if(cur_node == NULL)
        return -1;
    int n = 0;
...
template <typename T>
Node<T> *LinkedList<T>::CurNode()					// 返回当前结点的地址（NULL表示无当前结点）
{
    return cur_node;
}

template <typename T>
T & LinkedList<T>::CurData() throw(int)			// 引用返回当前结点数据域成员，函数调用表达式可做左值
{
    if(cur_node!=NULL)
...
template <typename T>
Node<T> *LinkedList<T>::getTop()					// 当前结点绝对定位：首结点、第n个结点(n从0起)、尾结点
{
    return cur_node = head;
}

template <typename T>
Node<T> *LinkedList<T>::get(int n)					// 当前结点绝对定位：首结点、第n个结点(n从0起)、尾结点
{
    if(n<0 || n>num)
        return cur_node=NULL;
    int i=0;
...
template <typename T>
Node<T> *LinkedList<T>::getBottom()				// 当前结点绝对定位：首结点、第n个结点(n从0起)、尾结点
{
    if(head==NULL)								// 空链表时
        cur_node = NULL;
...
template <typename T>
Node<T> *LinkedList<T>::Skip(int n) throw(int)	// 当前结点相对定位（n可为负数），默认n为1表示下一个结点
{
    if(cur_node == NULL)
        throw -1;
    int m = CurPos() + n;
    return cur_node = get(m);
}

template <typename T> template <typename TYPE>
Node<T> *LinkedList<T>::Locate(const TYPE &x, bool newsearch) // 当前结点依条件（给定TYPE类型）定位
{
...
template <typename T> template <typename TYPE>
void LinkedList<T>::Sort(const TYPE &x, bool ascending)	// 根据TYPE类型排序（升序或降序）
{
...
template <typename T>
void LinkedList<T>::Reverse()						// 链表结点倒置
{
...
#endif //ANDROIDTEST_LINKEDLIST_H
