...
#ifndef ANDROIDTEST_BLOCKBUFFER_H
#define ANDROIDTEST_BLOCKBUFFER_H
#pragma once
#include <assert.h>
#include <stdlib.h>
#include <string.h>
...
// boost::pool min accord
    template <unsigned BlockSize> struct default_block_allocator_malloc_free
    {
        enum
        {
    ...
    template <unsigned BlockSize> struct default_block_allocator_new_delete
    {
        enum
        {
            requested_size = BlockSize
...
typedef default_block_allocator_new_delete<32 * 1024> def_block_alloc_32k;
    ...
    template <typename BlockAllocator = def_block_alloc_4k, unsigned MaxBlocks = 2> class BlockBuffer
    {
    public:
    ...
    inline bool BlockBuffer<BlockAllocator, MaxBlocks>::resize(size_t n, char c)
    {
        ...
        {
            size_t len = n - size();
            if (!increase_capacity(len)) return false;
            memset(tail(), c, len);
        }
        m_size = n;
        return true;
    ...
    inline bool BlockBuffer<BlockAllocator, MaxBlocks>::replace(size_t pos, const char* rep, size_t n)
    {
        ...
        if (n > 0) memmove(m_data + pos, rep, n);
        return true;
    }
 ...
 */
    ...
    inline bool BlockBuffer<BlockAllocator, MaxBlocks>::increase_capacity(size_t increase_size)
    {
        ...
        if ((increase_size % allocator::requested_size) > 0) newblock++;
        ...
        if (s_current_total_blocks > s_peak_total_blocks) s_peak_total_blocks = s_current_total_blocks;
    ...
    template <typename Buffer> class ForwardBuffer
    {
    public:
        explicit ForwardBuffer(Buffer& sb)
                : m_sb(&sb)
                , m_pos(0)
        {
        }
...
#endif //ANDROIDTEST_BLOCKBUFFER_H
